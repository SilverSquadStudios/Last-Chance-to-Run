...
public class GeneratingPlatform : MonoBehaviour {
	...
	void Update () 
	{
		if(transform.position.x < Point.position.x)
		{
			...
			{
				TheSpikeGenerator.SpawnSpike(new Vector2 (transform.position.x+Random.Range (-1, 1), transform.position.y));

			}
			else if (Random.Range (0f, 100f) < ChanceToSpawnSlowDownPowerUp) 
			{
				ThePowerUpGenerator.SpawnSlowDown(new Vector2 (transform.position.x+ Random.Range (-1.5f,1.5f), transform.position.y+ 1f));

			}
			else if (Random.Range (0f, 100f) < ChanceToSpawnAHeart) 
			{
				TheHealthgenerator.SpawnLife(new Vector2 (transform.position.x+ Random.Range (-1.5f, 1.5f), transform.position.y+ 1f));
